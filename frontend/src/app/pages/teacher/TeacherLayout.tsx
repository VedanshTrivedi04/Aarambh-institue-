...
export function TeacherLayout() {
  ...
  useEffect(() => {
    if (!isLoading && (!isAuthenticated || (user && user.role !== 'teacher' && user.role !== 'admin'))) {
      navigate("/login");
    }
  ...
  const getPageTitle = () => {
    ...
    if (path.includes("notes")) return "Notes & Study Material";
  ...
  const userName = user ? `${user.first_name} ${user.last_name}`.trim() || user.email : "Teacher";
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <div className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-100 text-blue-600 border border-blue-200 inline-block">
                Teacher Panel
              </div>
            </div>
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200 bg-white">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-orange-100 text-orange-600 inline-block">
                {user?.email || "Teacher"}
              </div>
            </div>
          </div>
